# CircleObstacle Class

**Вложено элементов:** 7  

| CircleObstacle Class |
| --- |


# CircleObstacle Class


```csharp
public class CircleObstacle : pObstacle
```


```csharp
public class CircleObstacle : pObstacle
```


```csharp
public ref class CircleObstacle : public pObstacle
```


```csharp
public ref class CircleObstacle : public pObstacle
```

The CircleObstacle type exposes the following members.


|  | Name | Description |
| --- | --- | --- |
|  | CircleObstacle | Initializes a new instance of the CircleObstacle class |


|  | Name | Description |
| --- | --- | --- |
|  | Center | Center position of the obstacle |
|  | Description | Optional description or note for this obstacle. (Inherited from pObstacle .) |
|  | IsActive | Whether this obstacle has been added to a navigation mesh (Inherited from pObstacle .) |
|  | Radius | Radius of the circle |
|  | ReferenceId | The reference ID assigned by the navigation mesh (Inherited from pObstacle .) |
|  | Type | (Overrides pObstacle Type .) |


|  | Name | Description |
| --- | --- | --- |
|  | Equals | Determines whether the specified object is equal to the current object. (Inherited from Object .) |
|  | Finalize | Allows an object to try to free resources and perform other cleanup operations before it is reclaimed by garbage collection. (Inherited from Object .) |
|  | GetHashCode | Serves as the default hash function. (Inherited from Object .) |
|  | GetType | Gets the Type of the current instance. (Inherited from Object .) |
|  | MemberwiseClone | Creates a shallow copy of the current Object . (Inherited from Object .) |
|  | ToString | (Overrides pObstacle ToString .) |
|  | TryRemove | Removes this obstacle from the provided pathfinder if active. Returns true when removed and clears ReferenceId . (Inherited from pObstacle .) |

## Дочерние разделы
- [**CircleObstacle Constructor **](./CircleObstacle Constructor/README.md)
- [**CircleObstacle Properties**](./CircleObstacle Properties/README.md)
- [**CircleObstacle Methods**](./CircleObstacle Methods/README.md)
