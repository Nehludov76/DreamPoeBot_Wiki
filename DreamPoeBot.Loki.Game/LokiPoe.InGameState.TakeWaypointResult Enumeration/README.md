# LokiPoe.InGameState.TakeWaypointResult Enumeration

**Вложено элементов:** 0  

| LokiPoe InGameState TakeWaypointResult Enumeration |
| --- |


# LokiPoe InGameState TakeWaypointResult Enumeration

[Missing <summary> documentation for "T:DreamPoeBot.Loki.Game.LokiPoe.InGameState.TakeWaypointResult"]


```csharp
public enum TakeWaypointResult
```


```csharp
public enum TakeWaypointResult
```


```csharp
public enum class TakeWaypointResult
```


```csharp
public enum class TakeWaypointResult
```


|  | Member name | Value | Description |
| --- | --- | --- | --- |
|  | None | 0 |  |
|  | ProcessHookManagerNotEnabled | 1 |  |
|  | AreaNotFound | 2 |  |
|  | UiNotOpen | 3 |  |
|  | WaypointNotUsable | 4 |  |
|  | CouldNotSwitchToPart | 5 |  |
|  | CouldNotSwitchToAct | 6 |  |
|  | WaypointControlNotVisible | 7 |  |
|  | InstanceManagerDidNotOpen | 8 |  |
|  | CouldNotJoinNewInstance | 9 |  |
|  | TooManyInstances | 10 |  |
|  | HideoutControlNotVisible | 11 |  |
|  | NoHideout | 12 |  |
|  | NoHideoutTooltip | 13 |  |
|  | LabyrinthControlNotVisible | 14 |  |
|  | MenagerieControlNotVisible | 15 |  |
