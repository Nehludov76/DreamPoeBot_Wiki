...
[Missing <summary> documentation for "T:DreamPoeBot.Loki.Game.Objects.Chest"]
...
```csharp
public ref class Chest : public NetworkObject
```
...
The Chest type exposes the following members.
...
|  | DropSlots | Return the nr op drops available for this chest. |
...
|  | IsCorrupted | Return true if the chest is corrupted. |
...
|  | IsIdentified | Return true if the chest is identified. |
|  | IsIgnoreHidden | (Inherited from NetworkObject .) |
|  | IsLabyrinthRollerTrap | (Inherited from NetworkObject .) |
|  | IsLarge | Return true if the chest is a large chest. |
|  | IsLegionInitiator | (Inherited from NetworkObject .) |
|  | IsLocked | Return true if the chest is locked. |
...
|  | IsOpened | Return true if the chest is open. |
|  | IsSilverDoor | (Inherited from NetworkObject .) |
|  | IsStompable | Return true if the chest is stompable (can be open using travel skills). |
|  | IsStrongBox | Return true if the chest is a strongbox. |
|  | IsTargetable | (Inherited from NetworkObject .) |
|  | IsUltimatumChallengeInteractable | (Inherited from NetworkObject .) |
|  | IsVaalVessel | Return true if the chest is a Vaal vessel. |
|  | IsValid | (Inherited from NetworkObject .) |
|  | Metadata | (Inherited from NetworkObject .) |
|  | Name | (Inherited from NetworkObject .) |
|  | OpeningDestroys | Return true if the chest is destroyed after opening. |
|  | OpensOnDamage | Return true if the chest can be opened by damage. |
|  | Position | (Inherited from NetworkObject .) |
|  | Quality | The actual Quality of the chest. |
|  | Rarity | Return the rarity of the chest. |
|  | Reaction | (Inherited from NetworkObject .) |
|  | Stats |  |
|  | Type | (Inherited from NetworkObject .) |
|  | UpgradeOptions | Return a list the Items that can be used to upgrade the chest |
...
|  | UpgradeChest | Click a specific upgrade option to upgrade the chest. |

## Дочерние разделы
- [**Chest Properties**](./Chest Properties/README.md)
- [**Chest Methods**](./Chest Methods/README.md)
